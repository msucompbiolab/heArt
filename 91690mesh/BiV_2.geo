...
Mesh.CharacteristicLengthFactor = 0.6 ;// Fine
...
Merge "91690_LV2_1.stl";
Merge "91690_RV2_1.stl";
Merge "91690_EP2_1.stl";

CreateTopology;

//ll[] = Line "*";
ll() = {newl};
//L_LV_base = newl; Compound Spline(L_LV_base) = ll[2];
//L_RV_base = newl; Compound Spline(L_RV_base) = ll[0];
//L_epi_base = newl; Compound Spline(L_epi_base) = ll[1];
...
LL_base = newll; Line Loop(LL_base) = {1, 2, 3};
S_base = news; Plane Surface(S_base) = {LL_base};
Physical Surface(4) = {S_base};
//
SL_wall = newsl; Surface Loop(SL_wall) = {1, 2, 3, S_base};
