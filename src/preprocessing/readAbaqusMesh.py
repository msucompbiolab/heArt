...
def readAbaqusMesh(mesh_filename,
    ...
    for line in mesh_file:
        ...
        if line.upper().startswith("*ELEMENT"):
            if ("TYPE=F3D4" in line.upper()) and (("quad" in elem_types) or ("all" in elem_types)):
                context = "reading elems"
                cell_vtk_type = vtk.VTK_QUAD
                cell_nb_points = 4
                cell = vtk.vtkQuad()
            elif ("TYPE=C3D4" in line.upper()) and (("tet" in elem_types) or ("all" in elem_types)):
                context = "reading elems"
                cell_vtk_type = vtk.VTK_TETRA
                cell_nb_points = 4
                cell = vtk.vtkTetra()
            elif ("TYPE=C3D8" in line.upper()) and (("hex" in elem_types) or ("all" in elem_types)):
                context = "reading elems"
