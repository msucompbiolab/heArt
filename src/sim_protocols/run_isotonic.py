...
def run_isotonic(IODet, SimDet):
    ...
    dt = 1.0
    ntpt = 800
    if "length" in list(SimDet.keys()):
        length = SimDet["length"]
    if "width" in list(SimDet.keys()):
        width = SimDet["width"]
    if "nelem" in list(SimDet.keys()):
        nelem = SimDet["nelem"]
    if "nload" in list(SimDet.keys()):
        nload = SimDet["nload"]
    if "applied load" in list(SimDet.keys()):
        applied_load = SimDet["applied load"]
    if "dt" in list(SimDet.keys()):
        dt = SimDet["dt"]
    if "ntpt" in list(SimDet.keys()):
        ntpt = SimDet["ntpt"]
    ...
    Fdisp = File(
        IODet["outputfolder"] + IODet["folderName"] + IODet["caseID"] + "/u_disp.pvd"
    )
