...
class CLmodel(object):
    def et(self): #for P_LA
        if (isLV):
            if (self.t_la <= 1.5*self.Tmax_la):
                out = 0.5*(math.sin((math.pi/self.Tmax_la)*self.t_la - math.pi/2) + 1)
            else:
                out = 0.5*math.exp((-self.t_la + (1.5*self.Tmax_la))/self.tau_la)
        else:
            if (self.t_ra <= 1.5*self.Tmax_ra):
                out = 0.5*(math.sin((math.pi/self.Tmax_ra)*self.t_ra - math.pi/2) + 1)
            else:
    ...
    def GetPLA(self, params):
        ...
        if (self.parameters["t"] < self.SimDet["HeartBeatLength"] - self.tdelay_la):
            ...
            self.t_la = self.parameters["t"] - self.SimDet["HeartBeatLength"] + self.tdelay_la #

        self.PLA = self.et() * self.Ees_la * (self.V_LA - self.V0_la) + \
                (1 - self.et()) * self.A_la*(math.exp(self.B_la*(self.V_LA - self.V0_la)) - 1)
        return self.PLA
    ...
    def GetPRA(self):
        self.PRA = self.et()*self.Ees_ra*(self.V_RA - self.V0_ra) + (1 - et())*self.A_ra*(math.exp(self.B_ra*(self.V_RA - self.V0_ra)) - 1.0)
        return self.PRA
