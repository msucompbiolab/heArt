...
def run_BiV_ClosedLoop(IODet, SimDet):
    ...
    scale_factor_mesh = IODet["scaling"]
    ...
    PJmesh_filename = IODet["PJmesh_filename"]
    EPfacet_filename = IODet["EPfacet_filename"]
    isLV = SimDet["isLV"]
    delTat = SimDet["dt"]
    #  - - - - - - - - - - - -- - - - - - - - - - - - - - - -- - - - - - -
    ...
    coords = mesh_ep.coordinates()
    coords[:] *= scale_factor_mesh    
    ...
    EpiBCid_ep = MeshFunction('size_t', mesh_ep, 3, mesh_ep.domains()) 
    ...
    EpiBCid_ep = MeshFunction("size_t", mesh_ep, os.path.join(directory_ep, EPfacet_filename))
    File(os.path.join(outputfolder, "facetfn_test.pvd")) << EpiBCid_ep
   ...
   #  - - - - - - - - - - - -- - - - - - - - - - - - - - - -- - - - - - -
    ...
    if(isunloading):
        ...
        for it in np.arange(0,nloadstep):
            ...
            F_ED.vector()[:] = (project(MEmodel_.GetFmat(), MEmodel_.TF, solver_type="mumps").vector().get_local()[:])
    ...
    while(1):
        ...
        if(state_obj.cycle > prev_cycle):
            F_ED.vector()[:] = project(MEmodel_.GetFmat(), MEmodel_.TF).vector().get_local()[:]
