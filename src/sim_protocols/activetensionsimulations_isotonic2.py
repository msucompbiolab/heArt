...
Tmaxarray = [
    ...
    510e3,
    5000e3,
]  # , 500e3, 600e3]#, 700e3, 900e3]#, 100e3, 200e3, 300e3]
...
for Tmax_ in Tmaxarray:
    ...
    ploadarray = [
        1000
    ]  # , 10, 10]#np.linspace(100,5000,5)#[100,1000, 5000, 10000, 15000]
    ...
    for a_pas_val, pl in zip(a_pas_array, ploadarray):
        ...
        for tpt in np.arange(0, ntpt):
            ...
            pload_passiveW1 = (
                assemble((project(f0[i] * (PK1W1[i, j]) * f0[j], QDG)) * ds(2))
                * lbda[0]
            )
            passiveW1_load_array.append(pload_passiveW1)

            pload_passiveW4f = (
                assemble((project(f0[i] * (PK1W4f[i, j]) * f0[j], QDG)) * ds(2))
                * lbda[0]
            )
            passiveW4f_load_array.append(pload_passiveW4f)

            pload_passiveW4s = (
                assemble((project(f0[i] * (PK1W4s[i, j]) * f0[j], QDG)) * ds(2))
                * lbda[0]
            )
            passiveW4s_load_array.append(pload_passiveW4s)

            pload_passiveW8 = (
                assemble((project(f0[i] * (PK1W8[i, j]) * f0[j], QDG)) * ds(2))
                * lbda[0]
            )
            passiveW8_load_array.append(pload_passiveW8)

            pload_passivevol = (
                assemble((project(f0[i] * (PK1vol[i, j]) * f0[j], QDG)) * ds(2))
                * lbda[0]
            )
            passivevol_load_array.append(pload_passivevol)

            total_load_array.append(
                pload_active
                + pload_passiveW1
                + pload_passiveW4f
                + pload_passiveW4s
