...
sys.path.append("/mnt/home/hagersan/src")
...
from ..ep.EPmodel_pj import EPmodel
from ..mechanics.MEmodel_pj import MEmodel
...
def run_BiV_ClosedLoop(IODet, SimDet):
    ...
    isLV = SimDet["isLV"]
    delTat = SimDet["dt"]
    PJ_meshfilename = "paraview_line_LC.vtu"
    ploc_tol = SimDet["ploc_tol"]

    #####################################################################
    ########### Define  FHN  Problem for Purkinjee Fibres ###############
    #####################################################################
    # Convert line mesh in vtu to fenics mesh
    ugrid = vtk_py.readXMLUGrid(PJ_meshfilename)
    ncells = ugrid.GetNumberOfCells()
    npts = ugrid.GetNumberOfPoints()

    mesh_pj = Mesh()
    editor = MeshEditor()
    editor.open(mesh_pj, 1, 3)  # top. and geom. dimension are both 2
    ...
    for p in range(0, ncells):
        ...
        if pts.GetNumberOfIds() == 2:
            editor.add_cell(cnt, pts.GetId(0), pts.GetId(1))
            cnt = cnt + 1
    ...
    # Define BCS
    string = (
        "on_boundary && near(x[0], %18.10f, 1e-10) && near(x[1],%18.10f, 1e-10) && near(x[2], %18.10f, 1e-10)"
        % (term_nodes_coord[0][0], term_nodes_coord[0][1], term_nodes_coord[0][2])
    )
    AVnode = CompiledSubDomain(string)
    boundaries = FacetFunction("size_t", mesh_pj)
    boundaries.set_all(0)
    AVnode.mark(boundaries, 1)

    #####################################################################
    ########### Define  FHN  Problem for Myocardiac Tissue ##############
    #####################################################################
    ...
    EpiBCid_ep = CellFunction("size_t", mesh_ep)
    ...
    EPparams = {
        ...
        "term_nodes_coord": term_nodes_coord,
        "EpiBCid": EpiBCid_ep,
        "f0": f0_ep,
        "s0": s0_ep,
        "n0": n0_ep,
    ...
    solver_FHN_pj = EPmodel_.Solver_pj()
    solver_FHN_ep = EPmodel_.Solver_ep()

    #####################################################################
    ######### Define  Mechanical Problem for Myocardiac Tissue ##########
    #####################################################################
    #  - - - - - - - - - - - -- - - - - - - - - - - - - - - -- - - - - - -
    # Mechanics Mesh
    mesh_me = Mesh()
    ...
    if isunloading:  # COMMENTED OUT FOR NOW: READABILITY
    ...
    if not isLV:  # COMMENTED OUT FOR NOW: READABILITY
    ...
    while 1:
        ...
        ###SMS add: for distal aortic compliance
        ...
        PLA = et(t_la, Tmax_la, tau_la) * Ees_la * (V_LA - V0_la) + (
            1 - et(t_la, Tmax_la, tau_la)
        ) * A_la * (math.exp(B_la * (V_LA - V0_la)) - 1)
        ...
        if not isLV:
            ...
            PRA = et(t_la, Tmax_ra, tau_ra) * Ees_ra * (V_RA - V0_ra) + (
                1 - et(t_la, Tmax_ra, tau_ra)
            ) * A_ra * (math.exp(B_ra * (V_RA - V0_ra)) - 1)
        ...
        if isrestart == 0:
            ...
            if not isLV:
                export.writeP(
                    MEmodel_, [Psv, PLV, Psa, PLA, Ppv, PRV, Ppa, PRA], state_obj.tstep
                )
                export.writeQ(
                    MEmodel_,
                    [Qav, Qmv, Qsa, Qsv, Qpvv, Qtv, Qpa, Qpv, Qlad, Qlcx, Qlvad],
                    state_obj.tstep,
                )
                ...
                export.writeQ(
                    MEmodel_, [Qav, Qmv, Qsa, Qsv, Qlad, Qlcx, Qlvad], state_obj.tstep
                )
