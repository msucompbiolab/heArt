...
try:
    ...
    import vtk
except ImportError:  # vtk_py3/vtk are preprocessing-only; solver runs on pre-built meshes
    vtk_py = None
    vtk = None
import numpy as np
import math as math
...
def add_EpiSegments2hdf5(inPut, outPut, fname, caseName):
    ...
    mesh = Mesh()
    meshname = caseName
    f = HDF5File(MPI.comm_world, inPut + fname + ".hdf5", "r")
    ...
    EpiBCid = defSubDomain_SurfaceSegment(
        mesh=mesh,
        ugrid=ugridLoc,
        matId=AHAid,
        facetBdry=EpiBCid,
        epiMarks=facetboundaries,
    )
    dolfin.File(outF + "/" + casename + "_epiFacets" + ".pvd") << EpiBCid

    f = HDF5File(MPI.comm_world, outPut + fname + ".hdf5", "a")
...
def add_Segments2hdf5(inPut, outPut, fname, caseName):
    ...
    # f = HDF5File(MPI.comm_world, outPut+ fname+".hdf5", 'w')
    # f.write(mesh, meshname)
...
def partitionmeshforEP(ugrid, nsector=6, nz=2, meas=None, center=None, xaxis=[1, 0]):
    ...
    for cellid in range(0, ugrid.GetNumberOfCells()):
        ...
        # Partition ids are built with 1-based bins (argmax returns first True index), then flattened to regloc.
        ...
        regloc = (zloc - 1) * nsector + sloc  # 1..(nz*nsector)
...
def _cellwise_marker_function(mesh, markers, selector):
    # Avoid JIT-compiled Expressions by materializing DG0 coefficients in Python.
    V = FunctionSpace(mesh, "DG", 0)
    func = Function(V)
    local = func.vector().get_local()
    dm = V.dofmap()
    marker_values = markers.array()
    for cell in cells(mesh):
        dof = dm.cell_dofs(cell.index())[0]
        local[dof] = selector(marker_values[cell.index()])
    func.vector().set_local(local)
    func.vector().apply("insert")
    return func


def defMaterialProperty1(dolfin_mesh, mId):
    matProp = _cellwise_marker_function(
        dolfin_mesh, mId, lambda marker: 12.0 if marker == 7 else 0.0
    )
    dolfin.File(meshname + "_matProp" + ".pvd") << matProp
...
def defCPP_Matprop(mesh, mId, k):
    ...
    marker_set = {5, 6}
    kappa = _cellwise_marker_function(
        mesh,
        mId,
        lambda marker: k["kabNormal"] if marker in marker_set else k["kNormal"],
    )
...
def defCPP_Matprop_DIsch(mesh, mId, k):
    ...
    # Same selector as defCPP_Matprop(), but used for diffusion-like parameters (ischemia vs normal).
    marker_set = {5, 6}
    kappa = _cellwise_marker_function(
        mesh,
        mId,
        lambda marker: k["kIschemia"] if marker in marker_set else k["kNormal"],
    )
...
def defCPP_LBBB_Matprop(mesh, mId, meshname="CRT27_AS_smooth_fine"):
    marker_set = {14, 15, 24, 25, 26}
    kappa = _cellwise_marker_function(
        mesh, mId, lambda marker: 12.0 if marker in marker_set else 1.0
    )
    dolfin.File(meshname + "_matProp2_LBBB" + ".pvd") << kappa


def defCPP_Ischemia_Matprop(mesh, mId, meshname="CRT27_AS_smooth_fine"):
    kappa = _cellwise_marker_function(
        mesh, mId, lambda marker: 12.0 if marker <= 10 else 1.0
    )
    dolfin.File(meshname + "_matProp2_Ischemia" + ".pvd") << kappa
