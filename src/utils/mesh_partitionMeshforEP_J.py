...
def defMaterialProperty1(dolfin_mesh, mId):
    ...
    dolfin.File(meshname + "_matProp" + ".pvd") << interpolate(
        matProp, FunctionSpace(dolfin_mesh, "DG", 0)
    )
...
def defCPP_LBBB_Matprop(mesh, mId, meshname="CRT27_AS_smooth_fine"):
    ...
    dolfin.File(meshname + "_matProp2_LBBB" + ".pvd") << interpolate(
        kappa, FunctionSpace(mesh, "DG", 0)
    )
...
def defCPP_Ischemia_Matprop(mesh, mId, meshname="CRT27_AS_smooth_fine"):
    ...
    dolfin.File(meshname + "_matProp2_Ischemia" + ".pvd") << interpolate(
        kappa, FunctionSpace(mesh, "DG", 0)
    )
