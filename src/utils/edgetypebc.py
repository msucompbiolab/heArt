...
class _EdgeTypeBC(SubDomain):
    def __init__(self, rfun, marker):
        ...
        # marked edges
        # Python fallback: precompute edge endpoint coordinates for pointwise `inside()` checks.
...
def pick_endoring_bc(method="cpp"):
    if method == "cpp":
        # C++ SubDomain is significantly faster for pointwise BCs on many edges.
        module = compile_cpp_code(
            _endoring_code, additional_system_headers=["dolfin/mesh/SubsetIterator.h"]
        )
        return module.EdgeTypeBC
    else:
        # Pure-Python SubDomain is intended for debugging/portability; can be slow on large meshes.
        return _EdgeTypeBC
