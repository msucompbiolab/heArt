...
def extract_PV(filename, BCL, ncycle, isall):
	...
	for row in reader:
		...
		#RVP_array.append(float(row[3])) #vahid (c)
		#RVV_array.append(float(row[4])) #vahid (c)
		try:
	...
	#RVP_array = np.array(RVP_array) #vahid (c)
	#RVV_array = np.array(RVV_array) #vahid (c)
	...
	#RVP_array[ind], RVV_array[ind], Qmv_array[ind] #vahid (c)
...
def extract_Q(filename, BCL, ncycle):
	...
	for row in reader:
		...
		Qlvad_array.append(float(row[9]))
	        Qlcx1_array.append(float(row[10]))
                Qlcx2_array.append(float(row[11]))
                Qlcx3_array.append(float(row[12]))
                Qlad1_array.append(float(row[13]))
                Qlad2_array.append(float(row[14]))
                Qlad3_array.append(float(row[15]))
                Qrca1_array.append(float(row[16]))
                Qrca2_array.append(float(row[17]))
                Qrca3_array.append(float(row[18]))
	...
	Qlvad_array = np.array(Qlvad_array)
        ...
        Qlcx1_array = np.array(Qlcx1_array)
        Qlcx2_array = np.array(Qlcx2_array)
        Qlcx3_array = np.array(Qlcx3_array)
        Qrca1_array = np.array(Qrca1_array)
        Qrca2_array = np.array(Qrca2_array)
        Qrca3_array = np.array(Qrca3_array)
		
	ind = np.where(np.logical_and(tpt_array >= ncycle*BCL, tpt_array <= (ncycle+1)*BCL))
	
	return tpt_array[ind], Qav_array[ind], Qmv_array[ind], Qsa_array[ind], Qsv_array[ind], Qpvv_array[ind], Qtv_array[ind], Qpa_array[ind], Qpv_array[ind], Qlvad_array[ind], Qlad1_array[ind], Qlad2_array[ind], Qlad3_array[ind], Qlcx1_array[ind], Qlcx2_array[ind],Qlcx3_array[ind], Qrca1_array[ind], Qrca2_array[ind], Qrca3_array[ind]
...
def extract_P(filename, BCL, ncycle):
	...
	for row in reader:
		...
		PLA_array.append(float(row[4])) #vahid (c) the rest are removed for LV
	
	tpt_array = np.array(tpt_array)
	Psv_array = np.array(Psv_array)
	PLV_array = np.array(PLV_array)
	Psa_array = np.array(Psa_array)
	PLA_array = np.array(PLA_array) #vahid (c) the rest are removed for LV
	
	ind = np.where(np.logical_and(tpt_array >= ncycle*BCL, tpt_array <= (ncycle+1)*BCL))
	
	return tpt_array[ind], Psv_array[ind], PLV_array[ind], Psa_array[ind], PLA_array[ind] #vahid (c) the rest are removed for LV
...
def extract_probe(filename, BCL, ncycle):
	...
	for row in reader:
		...
		array_4.append(float(row[4]))
                array_5.append(float(row[5]))
                array_6.append(float(row[6]))	
		array_7.append(float(row[7]))
                array_8.append(float(row[8]))
                array_9.append(float(row[9]))	
	...
	#return tpt_array[ind], [array_1[ind], array_2[ind], array_3[ind], array_4[ind]]
        return tpt_array[ind], [array_1[ind], array_2[ind], array_3[ind], array_4[ind], array_5[ind], array_6[ind], \
               array_7[ind], array_8[ind], array_9[ind]]
