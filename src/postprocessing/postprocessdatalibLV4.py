...
def extract_PV(filename, BCL, ncycle, isall):
    ...
    if isall:
        ind = np.where(
            np.logical_and(tpt_array >= 0 * BCL, tpt_array <= (ncycle + 1) * BCL)
        )
    else:
        ind = np.where(
...
def extract_P(filename, BCL, ncycle):
    ...
    return (
        ...
        Psa_array[ind],
        PLA_array[ind],
    )  # vahid (c) the rest are removed for LV
...
def getradialposition(ugrid, LVendo, RVendo, Epi):
    ...
    for ptid in range(0, ugrid.GetNumberOfPoints()):
        ...
        closestLVendopt = np.array(
            LVendo.GetPoints().GetPoint(LVendo_ptlocator.FindClosestPoint(point))
        )
        closestRVendopt = np.array(
            RVendo.GetPoints().GetPoint(RVendo_ptlocator.FindClosestPoint(point))
        )
...
def getpointclouds(directory, LVendo, RVendo, Epi, matid):
    ...
    matid_array = [
        radpos.GetPointData().GetArray("region_id").GetValue(p)
        for p in range(0, radpos.GetNumberOfPoints())
    ]
...
def setMaterialRegion(
    directory, filebasename, fieldvariable, isparallel, LVendo, RVendo, Epi
):
