...
def postprocessdata(IODet, SimDet):
    ...
    for ncycle in range(cycle-1,cycle):
        ...
        tptt, LVP, LVV, Qmv = extract_PV(filename, BCL, ncycle)

        filename = directory+casename+"/"+"BiV_P.txt"
        tptt, Psv, PLV, Psa, PLA = extract_P(filename, BCL, ncycle) #v cut-off some variables next line
        #Ppv, PRV, Ppa, PRA, PinL, PinR
        ...
        ptcloud, radialpos, vtkradialpos = getpointclouds(directory+casename+"/", clipoffset=1e-5, npts=10000)
        ...
        #print("time point =", tptt[ np.argmax(PLV)], " imp = ", max(imp), " maximum PLV = ", PLV[np.argmax(PLV)]*0.0075)
        #print "imp = ", len(imp)
        #print "matid_arr = ", len(matid_arr)

        #imp_VTK_data = numpy_support.numpy_to_vtk(num_array=imp, deep=True, array_type=vtk.VTK_FLOAT)
        ...
        peakEff = np.abs(np.max(Eff, axis=1)*1)
        print("Eff:", peakEff)
