...
def extract_P(filename, BCL, ncycle):
	...
	for row in reader:
		...
		PRA_array.append(float(row[8]))
	        PinL_array.append(float(row[9]))
                PinR_array.append(float(row[10]))
	...
	PRA_array = np.array(PRA_array)
        PinL_array = np.array(PinL_array)
        PinR_array = np.array(PinR_array)
	
	ind = np.where(np.logical_and(tpt_array >= ncycle*BCL, tpt_array <= (ncycle+1)*BCL))
	
	return tpt_array[ind], Psv_array[ind], PLV_array[ind], Psa_array[ind], PLA_array[ind], Ppv_array[ind], PRV_array[ind], Ppa_array[ind], PRA_array[ind], PinL_array[ind], PinR_array[ind]
