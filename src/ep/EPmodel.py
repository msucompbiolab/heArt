...
class EPmodel(object):
    def __init__(self, params):
        ...
        # Mixed EP state: transmembrane potential in CG1, recovery variable in DG0 for robustness.
    ...
    def calculateDmat(self, f0, mesh, mId):
        ...
        i, j = indices(2)
        # Diffusion tensor: isotropic baseline + fiber-aligned enhancement.
    ...
    def MarkStimulus(self):
        ...
        EpiBCid_ep = MeshFunction("size_t", mesh_ep, mesh_ep.topology().dim() - 1)
        EpiBCid_ep.set_all(0)

        # Each `ploc_` defines a spherical region marked with a (1-based) stimulus label.
        for ploc_ in ploc:
    ...
    def Problem(self):
        ...
        # Start from rest to avoid carrying stale state across restarts unless explicitly restored.
        phi0 = interpolate(Constant(0.0), W_ep.sub(0).collapse())
        r0 = interpolate(Constant(0.0), W_ep.sub(1).collapse())
        ...
        # Time normalization rescales dt into the nondimensional FHN time unit.
        fhn_timeNormalizer = 12.9
        k = state_obj.dt.dt / fhn_timeNormalizer
        f_phi = f_phi(phi, r)
        f_r = f_r(phi, r)

        # AHAid is threaded for future material-dependent diffusion; current implementation uses `f0` only.
        D_tensor = self.calculateDmat(f0_ep, mesh=mesh_ep, mId=AHAid_ep)
        Dmat = D_tensor

        if self.isPK:
            self.max_pace_label = int(max(EpiBCid_ep.array()))
            printout(f"max_pace_label local: {self.max_pace_label}", mesh_ep.mpi_comm())
            max_value = MPI.comm_world.allreduce(self.max_pace_label, op=pyMPI.MAX)
            printout(f"max_pace_label global: {max_value}", mesh_ep.mpi_comm())
        ...
        for p in np.arange(0, self.max_pace_label):
            self.fstim_array.append(Constant(0.001))
        ...
        # Backward Euler in time with nonlinear reaction terms and anisotropic diffusion for `phi`.
        self.F_FHN = (
        ...
        for fstim in self.fstim_array:
            # Stimulus is applied on labeled regions (facet markers by default; volume markers for PK mode).
            if self.isPK:
    ...
    def UpdateVar(self):
        ...
        # Piecewise-constant pacing schedule: each label has (start_time, duration) in cycle-local time.
    ...
    def interpolate_potential_ep2me_phi(self, V_me):
        ...
        # Fast interpolation between non-matching meshes without an explicit projection solve.
        LagrangeInterpolator.interpolate(V_me, self.getphivar())

        return V_me

    def reset(self):
        # Hard reset of EP state (used at cycle boundaries when periodicity checks are enabled).
        phi0 = interpolate(Constant(0.0), self.W_ep.sub(0).collapse())
        r0 = interpolate(Constant(0.0), self.W_ep.sub(1).collapse())
