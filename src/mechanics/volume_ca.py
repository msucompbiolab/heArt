...
#        dim = W_.sub(0).dim()
        ...
        u_coord = u_.function_space().tabulate_dof_coordinates().reshape(-1, n) # dim --> -1
...
#            lst_v2d.append(b[3 * i][1] / 3)
        ...
        d2v_A_n = d2v_A_n[xrange(0, len(d2v_A_n), 3)]/3
        ...
        for i in range(self.mesh.num_vertices()):
            ...
            b_i = np.array([int(3 * map_dofs_u[i][0]), int(3 * map_dofs_u[i][0] + 1), int(3 * map_dofs_u[i][0] + 2)])
            c_i = np.array([i, self.mesh.num_vertices() + i, 2 * self.mesh.num_vertices() + i])
...
#        ds_x = Measure("ds", domain = self.mesh_n, subdomain_data = self.facet_function_n, subdomain_id = self.sub_id)
        ds_x = Measure("ds", domain = self.mesh_t, subdomain_data = facet_t, subdomain_id = self.sub_id)
...
#        vol = assemble(1.0 * ds_x)
        ...
        vol_form = -Constant(1.0/3.0) * inner(det(F)*dot(inv(F).T, N), X + a_n) * ds_x
...
#        vertex_coor = []
        ...
        for f in subset_iter:
            for v in vertices(f):
                v_idx = v.index()
                vertex_u_values.append((h_[v_idx], h_[self.mesh.num_vertices() + v_idx], h_[2*self.mesh.num_vertices() + v_idx]))
#                vertex_coor.append((v.point().x(), v.point().y(), v.point().z()))
                num_vertices += 1

        uc_x = 0; uc_y = 0; uc_z = 0
        for u_x, u_y, u_z in vertex_u_values:
            uc_x += u_x; uc_y += u_y; uc_z += u_z
...
#        centroid_x, centroid_y, centroid_z = self.u_centroid(u_, W_) #vahid comment alternative is just to set zeros
        ...
        edges = [f.entities(0) for f in SubsetIterator(self.edge_boundaries, self.sel_label)]
        ...
        editor_n.add_vertex(prev_num_vertices, np.array([centroid_x, centroid_y, 0.0], dtype=np.double))

        editor_n.add_vertex(prev_num_vertices + 1, np.array([centroid_x, centroid_y, offset], dtype=np.double))
