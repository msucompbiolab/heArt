...
#from volume_ca import MeshModifier
...
#        Velem = VectorElement("CG", self.mesh_me.ufl_cell(), 1, quad_scheme="default") #new_v
        Qelem = FiniteElement("CG", self.mesh_me.ufl_cell(), 1, quad_scheme="default")
#        Qelem = FiniteElement("DG", self.mesh_me.ufl_cell(), 0, quad_scheme="default") #new_v
        ...
        if(self.isincomp):
            if(self.isLV):
                if("springbc" in self.SimDet.keys() and self.SimDet["springbc"]):
                    ...
                    self.W = FunctionSpace(self.mesh_me, MixedElement([Velem, Qelem, VRelem])) #new
            else:
                if("springbc" in self.SimDet.keys() and self.SimDet["springbc"]):
    ...
    def unloading_pres(self, params):
        default_params = {"EDP":12, "maxit":20, "restol":1e-3, "drestol":1e-4, "EDPtol": 1e-1, "preinc":1, "LVangle":[60,-60]}
    ...
    def found_edges(self):
        ...
        facet_boundaries = self.facetboundaries_me
        edge_boundaries = self.edgeboundaries_me

        id_sel = 1
        subset_iter = SubsetIterator(edge_boundaries, id_sel)
        for f in subset_iter:
            for v in vertices(f):
                x, y, z = v.point().x(), v.point().y(), v.point().z()

        id_sel_ = 1
        iter_ = SubsetIterator(facet_boundaries, id_sel_)

        facet_function_ = FacetFunction("size_t", mesh_me)

        for g in iter_:
            for facet in facets(g):
                vertex_ = facet.entities(0)
                Z_coord = sum(mesh_me.coordinates()[vertex][2] for vertex in vertex_) / 3.0
                if abs(Z_coord) > 8.0:
    ...
    def set_BCs(self):
        ...
        bcbot = DirichletBC(W.sub(0).sub(2), Expression(("0.0"), degree = 2), facet_, 3)

        bctop = DirichletBC(W.sub(0), Expression(("0.0", "0.0", "0.0"), degree = 0), facetboundaries, topid)
...
#        PN_base = P_LV * norm(J * inv(Fmat.T) * N_me) * v_base
    ...
    def Problem(self):
        ...
        if(isincomp):
            if(self.isLV):
                if("springbc" in self.SimDet.keys() and self.SimDet["springbc"]):
                    du, dp = TrialFunctions(W_me) #new / all below
...
#            int_exp = interpolate(expr_, W_me.sub(0).sub(2).collapse())

            ## epicardial
            Kepi_n = 2e5; Kepi_t = 2e4
            F3_epi = Kepi_n * inner(outer(N_me, N_me) * u_me, v_me) * ds_me(epiid) + \
                    Kepi_t * inner((Identity(u_me.ufl_shape[0]) - outer(N_me, N_me)) * u_me, v_me) * ds_me(epiid)
            ...
            Fs = 1.0e-10 * (p_me - p_me/cell_volume) * (q_me - q_me/cell_volume) * dx_me
            Ftotal += Fs
    ...
    def GetDisplacement(self):
        ...
        if(self.isLV):
            ...
            if("springbc" in self.SimDet.keys() and self.SimDet["springbc"]):
                u, p = self.w_me.split(deepcopy=True) #new
                rv_pendo = []
            else:
        ...
        return u
    def Getd2v(self):
        return dof_to_vertex_map(self.W)
...
#        return self.uflforms.LVcavitypres()
