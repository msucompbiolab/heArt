...
class Forms(object):
    ...
    def volume_computation(self): #for volume computation
        ...
        ds_ = Measure("ds", domain=mesh, subdomain_data = self.parameters["facetboundaries"], subdomain_id = self.parameters["topid"])
...
#        vol_form = -Constant(1.0/3.0) * inner(det(F)*dot(inv(F).T, N), X + u)*ds(self.parameters["LVendoid"])
        ...
        vol_form = -Constant(1.0/3.0) * inner(det(F)*dot(inv(F).T, N), X + u - b)*ds(self.parameters["LVendoid"])
    ...
    def springbc(self): #for v_base computation
        ...
        JFN_x = assemble(JFN[0] * ds(self.parameters["LVendoid"]))
        JFN_y = assemble(JFN[1] * ds(self.parameters["LVendoid"]))
        JFN_z = assemble(JFN[2] * ds(self.parameters["LVendoid"]))
