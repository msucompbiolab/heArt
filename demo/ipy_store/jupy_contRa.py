...
data = scipy.io.loadmat(os.path.join(os.curdir, 'raw_data', 'Rat_23019_constQbpm_hemodynamics.mat'))
#cleaning up
...
d_dlvv = {'twoc': 0.0, 'threec': 0.02, 'fourc': 0.04, 'fivec': 0.06,
            'sixc': 0.08, 'sixcfourg': 0.09, 'sevenc': 0.1, 'sevencfourg': 0.11,
            'eightc': 0.12, 'eightcfourg': 0.13, 'ninec': 0.14, 'ninecfourg': 0.15, 'tenc': 0.16}
...
init_case = None; init_lvv = None;
init_Cparam = None;
init_bff = None; init_bfx = None; init_bxx = None
...
prev_Cparam = 11.7
prev_bff = 49.63; prev_bfx = 22.76; prev_bxx = 45.85
...
#    content = re.sub(r'\b' + re.escape(str(init_Cparam[0])) + r'\b', str(prev_Cparam), content)
#
#if content.find('bff') != -1:
#    content = re.sub(r'\b' + re.escape(str(init_bff[0])) + r'\b', str(prev_bff), content)
#
#if content.find('bfx') != -1:
#    content = re.sub(r'\b' + re.escape(str(init_bfx[0])) + r'\b', str(prev_bfx), content)
#
#if content.find('bxx') != -1:
#    content = re.sub(r'\b' + re.escape(str(init_bxx[0])) + r'\b', str(prev_bxx), content)
...
#while True:
#
#    with open(os.path.join(os.curdir, script_f), 'w') as ofile:  # finalize script_f to run
#        ofile.write(content)
#    # pdb.set_trace()
#
...
#            d_lvp[preload][2] = float(ifile.readline().split()[2]) * 7.5 / 1.e3
#    except FileNotFoundError:
...
#
#    if content.find('Passive params') != -1:
#        content = re.sub(r'\b' + re.escape(str(prev_Cparam)) + r'\b', str(Cparam), content)
#    prev_Cparam = Cparam
##    pdb.set_trace()
#
## pdb.set_trace()
...
#    try:
#        with open(os.path.join(match.group(1), preload, 'BiV_P.txt'), 'r') as ifile:
#            edp_sim = float(ifile.readline().split()[2]) * 7.5 / 1.e3
#    except FileNotFoundError:
#        print('File doesn\'t exist for preload', preload)
#
...
#    if (0.8 < b_coeff < 1.2):
#        print('bff is: ' + str(prev_bff) + ' and bfx is: ' + str(prev_bfx) + ' and bxx is: ' + str(prev_bxx))
#        break
#
...
#
#    print('next bff is: ' + str(bff) + ' next bfx is: ' + str(bfx) + ' and next bxx is: ' + str(bxx))
#
#    if content.find('bff') != -1:
...
for preload in d_lvp.keys():
    ...
    while True:
        ...
        l_lvp = [float(line.split()[2]) for line in lines]
        endsys = max(l_lvp)
        endsys_sim = endsys * 7.5 / 1.e3
    ...
    dict_contRa[preload] = prev_contRactility
#    pdb.set_trace()

# pdb.set_trace()
# for key in dict_contRa.keys():
#    print('for preload ' + str(key) + ' contRa is: ' + str(dict_contRa[key][0]))
...
with open(file_path, "w") as json_file:
    json.dump(dict_contRa, json_file)
